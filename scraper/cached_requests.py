...
class CReq:
    def __init__(
        ...
        if engine is not None:
            ...
            self.dbs["pages"] = db.Table(
                "pages", self.dbs["metadata"], autoload=True, autoload_with=engine
            )
        ...
        while True:
            try:
                ...
                if num_tries == self.max_tries:
                    log.error(e)
                    raise RuntimeError(f"Failed to get page after {num_tries} tries.")
    ...
    def post_www(self, url, data, request_headers=None):
        ...
        while True:
            try:
                ...
                num_tries += 1
                if num_tries == self.max_tries:
                    raise RuntimeError(f"Failed to post page after {num_tries} tries.")
        return r

    def get_db_id(self, url, post_msg=None):
        pages = self.dbs["pages"]
        query = (
            db.select([pages.columns.id, pages.columns.headers, pages.columns.date_created])
            .where(
                db.and_(
                    pages.columns.url == url,
                    pages.columns.status == 200,
                    pages.columns.post_msg
                    == (db.null() if post_msg is None else json.dumps(post_msg, sort_keys=True)),
                )
            )
                ...
                with self.dbs["engine"].connect() as connection:
                    query = (
                        ...
                        .values(
                            ...
                            post_msg=db.null()
                            if post_msg is None
                            else json.dumps(post_msg, sort_keys=True),
                        )
                        .returning(pages.columns.id)
                    )
    ...
    def get(self, url, max_age_days=365) -> List[object]:
        ...
        if self.dbs["engine"] is None:
            return self.get_www(url)
        ...
        if len(db_res):
            ...
            if db_res[0].status_code == 404 or db_res[0].date_created < (
                datetime.today() - timedelta(days=max_age_days)
            ):
                force_new = True
    ...
    def post(self, url, data: dict, request_headers=None, force_new=False):
        ...
        # if no engine specified, act like normal requests
        if self.dbs["engine"] is None:
            return self.post_www(url, data=data, request_headers=request_headers)

        res = None
        if not force_new:
            res = self.get_db(url, post_msg=data)
            if not len(res) or res[0].status_code == 404:
                force_new = True
            else:
                res = res[0]
